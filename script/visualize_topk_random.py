...
# -*- coding: utf-8 -*-
"""
可视化 Top-K DDPG 模型推测结果 (随机基站位置版本)
用于测试旧模型 (未固定基站位置)
"""
...
# ==================== Monkey Patching ====================
# 强制替换 TopKEnv 的 _init_bs_positions 方法，使其使用随机分布
# 即使 M=16，也会使用随机分布
def random_init_bs_positions(self):
    print("[Monkey Patch] Using RANDOM BS positions instead of fixed grid.")
    self.bs_positions = np.random.uniform(0, [config.X, config.Y], (config.M, 2))

# 应用 Patch
TopKEnv._init_bs_positions = random_init_bs_positions
# =========================================================

def get_args():
    parser = argparse.ArgumentParser(description='Visualize Top-K DDPG model inference (Random BS)')
...
def visualize(env, bs_positions, uav_positions, power_matrix, config):
    ...
    ax.set_title(f'Network Visualization (Random BS Positions)')
...
def main():
    ...
    # 1. 初始化环境 (使用 Monkey Patched 的 TopKEnv)
    env = TopKEnv()
    ...
    print(f"Environment: {args.env} (Random BS Mode)")
