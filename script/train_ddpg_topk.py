...
def get_args():
    parser = argparse.ArgumentParser(description="DDPG Top-K 训练脚本")
    ...
    parser.add_argument('--critic-lr', type=float, default=3e-4, help='Critic 学习率')
...
def setup_policy(args, env):
    ...
    # 创建 Actor 网络 (Top-K Model)
    # 注意: Top-K Actor/Critic 只需要 args, 内部会读取 config 中的 M, N, K_MAX
...
def run_training(args, env, train_envs, test_envs, policy, log_path):
        ...
        if env_step % 1000 == 0: # 每 1000 步记录一次
            ...
            if len(buffer) > 100:
                ...
                with torch.no_grad():
                    ...
                    # 记录 Actor 输出分布
                    actor_out, _ = policy._actor(obs) # Actor returns (action, state)
                    writer.add_scalar('debug/actor_power_mean', actor_out.mean().item(), env_step)
                    
                    # --- Calculate Loss & Std for Monitoring ---
                    # 1. Actor Loss (Approx)
                    # Calculate Q(s, a) for current policy
                    actor_q1, _ = policy._critic(obs, actor_out)
                    actor_loss = -actor_q1.mean()
                    actor_loss_std = actor_q1.std()
                    
                    writer.add_scalar('debug/actor_loss', actor_loss.item(), env_step)
                    writer.add_scalar('debug/actor_loss_std', actor_loss_std.item(), env_step)

                    # 2. Critic Loss (Approx)
                    # Calculate Target Q
                    batch_next_obs = torch.tensor(batch.obs_next, device=args.device, dtype=torch.float32)
                    batch_rew = torch.tensor(batch.rew, device=args.device, dtype=torch.float32).unsqueeze(-1)
                    batch_done = torch.tensor(batch.done, device=args.device, dtype=torch.float32).unsqueeze(-1)
                    
                    # Target Actor Action
                    target_act, _ = policy._target_actor(batch_next_obs)
                    # Add noise (TD3 style)
                    noise = torch.randn_like(target_act) * policy._policy_noise
                    noise = noise.clamp(-policy._noise_clip, policy._noise_clip)
                    target_act = (target_act + noise).clamp(0, 1)
                    
                    # Target Q Value
                    target_q1, target_q2 = policy._target_critic(batch_next_obs, target_act)
                    target_q = torch.min(target_q1, target_q2)
                    target_val = batch_rew + args.gamma * (1 - batch_done) * target_q
                    
                    # Current Q Value
                    current_q1, current_q2 = policy._critic(obs, act)
                    
                    # Critic Loss (MSE)
                    loss_q1 = F.mse_loss(current_q1, target_val, reduction='none')
                    critic_loss = loss_q1.mean()
                    critic_loss_std = loss_q1.std()
                    
                    writer.add_scalar('debug/critic_loss', critic_loss.item(), env_step)
                    writer.add_scalar('debug/critic_loss_std', critic_loss_std.item(), env_step)
                    # -------------------------------------------
    ...
    for epoch in range(args.warmup_epochs):
        ...
        for step in range(args.warmup_steps_per_epoch):
            ...
            # Record Warmup Loss to TensorBoard
            global_step = epoch * args.warmup_steps_per_epoch + step
            if step % 100 == 0:
                writer.add_scalar('warmup/critic_loss', res['loss/critic'], global_step)

            # Debug Print every 500 steps
            # 修改：与 Summary 对齐，在 (epoch + 1) % 5 == 0 时输出，即第 4, 9, 14... (index) 轮
            if step % 500 == 0 and (epoch + 1) % 5 == 0:
                obs = torch.tensor(batch.obs, device=args.device, dtype=torch.float32)
                act = torch.tensor(batch.act, device=args.device, dtype=torch.float32)
                ...
                print(f"\n[Warmup Debug] Epoch {epoch} (Index) Step {step}")
                ...
                print(f"  > Loss:   {res['loss/critic']:.6f} (Instantaneous)")
        ...
        if (epoch + 1) % 5 == 0:
            print(f"Warmup Epoch {epoch+1}/{args.warmup_epochs}: Critic Loss = {np.mean(losses):.6f} (Average)")
    ...
    print(f"\n{'='*20} Phase 1: Main Training (Top-K Compressed) {'='*20}")
