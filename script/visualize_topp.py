...
def get_args():
    parser = argparse.ArgumentParser(description='Visualize Top-P DDPG model inference')
    parser.add_argument('--run-name', type=str, required=True,
                        help='Run name (e.g., Dec10-123456)')
    parser.add_argument('--env', type=str, default='topp',
                        help='Environment name (default: topp)')
...
def get_env_and_config(env_name):
    ...
    try:
        if env_name == 'topp':
            ...
            import env.topp.config as config
            env = TopPEnv()
        else:
...
def load_model(run_name, state_dim, action_dim, config):
    ...
    possible_paths.extend([
        f'log/default/ddpg/topp/{run_name}/policy.pth',
    ...
    # 初始化网络 (注意：训练脚本 train_ddpg_topp.py 中使用的是 hidden_dim=256)
...
def infer_actions(policy, state, config):
    ...
    with torch.no_grad():
        ...
        # 连接矩阵由 Top-P 规则决定，这里我们无法直接从模型输出得到
        # 但我们可以从环境状态中推断，或者假设环境已经计算好了
        # 为了可视化，我们假设所有非零功率都是连接的，或者需要传入 env 来获取真实的 connection_matrix
        # 这里我们暂时返回全 1，稍后在 visualize 中修正，或者让 env 重新计算
        connection_actions = np.ones((M, N)) 
            
    return connection_actions, power_actions

def save_weights(policy, run_name):
    """保存模型权重到txt文件"""
    log_dir = f'log/default/ddpg/topp/{run_name}'
    ...
    with open(output_path, 'w') as f:
        f.write(f"Model Weights for DDPG Top-P (Run: {run_name})\n")
        f.write("="*80 + "\n\n")
...
def visualize(env, bs_positions, uav_positions, power_actions, config):
    """可视化"""
...
def main():
    ...
    # 4. 推测动作
    _, power_actions = infer_actions(policy, state, config)
    
    # 5. 应用动作到环境以计算奖励和更新状态
    # 注意：TopPEnv.step 会自动应用连接掩码
    # 我们需要手动将 power_actions 展平传入 step
    action_flat = power_actions.flatten()
    _, reward, _, _, _ = env.step(action_flat)
    
    # 获取经过环境处理后的真实功率矩阵（已掩码和归一化）
    final_power_matrix = env.power_matrix

    print(f"Power actions shape: {final_power_matrix.shape}")

    # 打印详细动作矩阵
    print("\nPower Actions (BS-UAV matrix) [After Masking & Norm]:")
    for m in range(config.M):
        row = [f"{final_power_matrix[m, n]:.2f}" for n in range(config.N)]
        print(f"BS{m}: {' '.join(row)}")
        
    print("\nConnection Matrix (Top-P Rule):")
    for m in range(config.M):
