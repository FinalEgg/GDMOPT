...
def simulate_td3(model_path, interval=1.5):
    # 创建环境、生成初始状态，并加载模型
    ...
    for k, v in state_dict.items():
        if k.startswith('_actor.') or k.startswith('actor.'):
            # 提取actor相关参数
            key = k.replace('_actor.', '').replace('actor.', '')
    ...
    action = predicted_action.detach().cpu().numpy().flatten()  # 添加flatten()来展平数组
...
if __name__ == "__main__":
    # 使用TD3模型路径
    model_path = "log/default/TD3/model-1/policy.pth"  # 替换为您的TD3模型路径
    simulate_td3(model_path)