...
class CellFreeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for m in range(M):
            total_power = np.sum(self.power_matrix[m, :])
            if total_power > 0:
                self.power_matrix[m, :] /= total_power
        ...
        for k in range(N):  # 对于每个无人机 k
            ...
            for m in Ak:  # 遍历服务用户 k 的基站 m
                ...
                for kp in range(N):  # 遍历所有用户 kp
                    if self.connection_matrix[m, kp] == 1 and kp != k:  # 如果基站 m 连接到用户 kp，且 kp != k（排除自身）
            ...
            # SINR 的分母：干扰 + 噪声
            denominator = pd * interference + NOISE_POWER
            # 信干噪比
